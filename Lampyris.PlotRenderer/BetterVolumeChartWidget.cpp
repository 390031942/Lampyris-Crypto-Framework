...
BetterVolumeChartWidget::BetterVolumeChartWidget(QWidget* parent) : GridChartWidget( parent) {
    setCellGridHeightMin(40);
    initialize();
}

void BetterVolumeChartWidget::paintEvent(QPaintEvent *event) {
...
void BetterVolumeChartWidget::initialize() {
    //读取数据
...
bool BetterVolumeChartWidget::readData(QString strFile) {
    return mDataFile.readData(strFile);
}

//获得画图所需的各项指标
void BetterVolumeChartWidget::getIndicator()
{
    maxVolume = 0;
...
void BetterVolumeChartWidget::drawYtick() {
    getIndicator();
    QPainter painter(this);
    QPen     pen;
    pen.setColor(Qt::red);
    painter.setPen(pen);
    double ystep = maxVolume / getHorizontalGridCount();
    QString str;
    for (int i = 0; i <= getHorizontalGridCount(); ++i) {
        str.sprintf("%d", (int)(i * ystep));
        painter.drawText(QPoint(getWidgetWidth() - getMarginLeft() + 10,
            getWidgetHeight() - getMarginBottom() - i * getCellGridHeight()),
            str);
    }
}

void BetterVolumeChartWidget::drawVolume()
{
    QPainter painter(this);
    QPen     pen;
...
void BetterVolumeChartWidget::drawAverageLine(int day){
