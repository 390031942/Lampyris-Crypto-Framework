...
public class QuoteManager:ILifecycle
{
    ...
    /* 运行的定时器任务id列表 */
    private List<int> m_PlannedTaskSchedulerIds = new List<int>();
    ...
    public override void OnUpdate()
    {
        if(!m_IsInitialized)
        {
            ...
            {
                // Tick 行情列表-合约
                m_PlannedTaskSchedulerIds.Add(PlannedTaskScheduler.AddTimeTask(() =>
                {
                }, 200, -1));
                ...
                m_PlannedTaskSchedulerIds.Add(PlannedTaskScheduler.AddTimeTask(() =>
                {
                    ...
                    if (DateTimeUtilEx.GetBarTimeSpanDiff(lastDt, curDt, BarSize._1m) != 0)
                    {
                ...
                // 市场最近的数据初始化后，启动 市场监测
                Logger.LogInfo("All recent quote downloads have finished, starting market monitoring.");
                m_PlannedTaskSchedulerIds.Add(PlannedTaskScheduler.AddTimeTask(() =>
                {
    ...
    {
        foreach(int id in m_PlannedTaskSchedulerIds)
        
            PlannedTaskScheduler.Clear(id);
        }
    }
}
